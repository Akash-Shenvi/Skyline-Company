...
const SkeletonCard: React.FC = () => (
    <div className="rounded-2xl border border-brand-surface bg-white p-6 animate-pulse">
        <div className="mb-3 h-10 w-10 rounded-lg bg-brand-surface" />
        <div className="mb-2 h-8 w-16 rounded-md bg-brand-surface" />
        <div className="h-4 w-28 rounded-md bg-brand-surface" />
    </div>
);

const SkeletonRow: React.FC = () => (
    <div className="flex items-center justify-between rounded-xl border border-brand-surface p-4 animate-pulse">
        <div className="space-y-2">
            <div className="h-4 w-36 rounded-md bg-brand-surface" />
            <div className="h-3 w-24 rounded-md bg-brand-surface" />
        </div>
        <div className="text-right space-y-2">
            <div className="h-4 w-20 rounded-md bg-brand-surface" />
            <div className="h-3 w-28 rounded-md bg-brand-surface" />
        </div>
    </div>
);
...
const StatCard: React.FC<{
    ...
    return (
        <motion.div
            ...
            className="group relative rounded-2xl border border-brand-surface/80 bg-white p-6 shadow-sm hover:shadow-xl transition-shadow overflow-hidden"
        >
            {/* Accent glow */}
            <div
            ...
            <p className="mb-1 text-3xl font-bold tracking-tight text-brand-black">{value}</p>
            <p className="text-sm font-semibold text-brand-olive-dark">{label}</p>
            {subtext && (
                <p className="mt-1.5 flex items-center gap-1 text-xs text-brand-olive-light">
    ...
    <motion.div variants={itemVariants} custom={delay}>
        <Link to={to} className="group block">
            <motion.div
                ...
                className="relative rounded-2xl border border-brand-surface/80 bg-white p-6 shadow-sm hover:shadow-lg transition-all overflow-hidden"
            ...
            >
                ...
                <div className="relative flex items-start justify-between">
                    <div className="flex-1">
                        ...
                        <h3 className="mb-1 text-base font-bold text-brand-black">{title}</h3>
                        <p className="text-sm text-brand-olive leading-relaxed">{description}</p>
                    </div>
                    <div
...
const SectionHeader: React.FC<{ icon: React.ReactNode; title: string; subtitle?: string }> = ({ icon, title, subtitle }) => (
    <div className="mb-6">
        ...
        {subtitle && <p className="pl-10 text-sm text-brand-olive">{subtitle}</p>}
    </div>
);
...
const SectionDivider: React.FC = () => (
    <div className="my-2 h-px bg-gradient-to-r from-transparent via-brand-surface to-transparent" />
);
    ...
    return (
        <AdminLayout>
            <div className="max-w-7xl mx-auto">
                {/* ── Hero ── */}
                <div className="relative bg-brand-black text-white py-24 sm:py-32 text-center overflow-hidden -mx-6 -mt-6 lg:-mx-8 lg:-mt-8 mb-10">
                    <HeroBackground />
                    ...
                    <div className="relative z-10 max-w-3xl mx-auto px-4">
                        <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
                            <div className="inline-flex items-center gap-2 rounded-full border border-brand-gold/30 bg-brand-gold/10 px-4 py-1.5 mb-5">
                                ...
                                <span className="text-xs font-semibold uppercase tracking-widest text-brand-gold">Admin Portal</span>
                            </div>
                            <h1 className="mb-3 text-4xl font-bold font-serif md:text-5xl">Dashboard</h1>
                            <p className="text-lg text-brand-olive-light max-w-md mx-auto">Overview of your academy's performance and operations.</p>
                        </motion.div>
                    </div>
                </div>
                ...
                <div className="space-y-10 pb-10">
                    ...
                    >
                        ...
                        />
                        <StatCard
                            icon={<BookOpen className="h-5 w-5 text-brand-olive" />}
                    ...
                    <div className="grid gap-6 lg:grid-cols-3">
                        ...
                        <motion.section
                            ...
                            className="lg:col-span-2 rounded-2xl border border-brand-surface/80 bg-white shadow-sm overflow-hidden"
                        >
                            <div className="flex items-center justify-between px-6 py-5 border-b border-brand-surface">
                                <div className="flex items-center gap-2.5">
                                    <TrendingUp className="h-5 w-5 text-brand-gold" />
                                    <h3 className="font-bold text-brand-black">Recent Classes</h3>
                                </div>
                                <Link to="/admin/batches" className="flex items-center gap-1 text-xs font-semibold text-brand-gold hover:underline">
                            ...
                            <div className="p-4 space-y-2">
                                <AnimatePresence>
                                    {loading ? (
                                        ...
                                        >
                                            <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-brand-surface">
                                                <BookOpen className="h-6 w-6 text-brand-olive-light" />
                                            </div>
                                            <p className="text-sm font-medium text-brand-olive">No active classes found</p>
                                            <p className="text-xs text-brand-olive-light mt-1">Classes will appear here once created</p>
                                        </motion.div>
                                    ) : (
                                        batches.slice(0, 5).map((batch, idx) => (
                                            <motion.div
                                                ...
                                                className="group flex items-center justify-between rounded-xl border border-brand-surface p-4 hover:border-brand-gold/40 hover:bg-brand-gold/[0.03] transition-all cursor-default"
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="h-9 w-9 rounded-lg bg-brand-gold/10 flex items-center justify-center shrink-0">
                                                        <BookOpen className="h-4 w-4 text-brand-gold" />
                                                    </div>
                                                    <div>
                                                        <p className="font-semibold text-sm text-brand-black">{batch.courseTitle}</p>
                                                        <p className="text-xs text-brand-olive-light">{batch.name}</p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="text-right shrink-0">
                                                        <p className="text-sm font-semibold text-brand-black">
                                                            {batch.students?.length || 0} <span className="text-xs font-normal text-brand-olive-light">students</span>
                                                        </p>
                                                        <p className="text-xs text-brand-olive-light mt-0.5">
                                                    ...
                                                    <Link
                                                        ...
                                                        className="inline-flex items-center gap-1.5 rounded-lg bg-brand-gold/10 px-3 py-1.5 text-xs font-semibold text-brand-gold border border-brand-gold/20 hover:bg-brand-gold/20 hover:border-brand-gold/40 transition-all duration-200 shrink-0"
                        ...
                        <motion.div
                            ...
                            className="space-y-4"
                        >
                            <div className="rounded-2xl bg-gradient-to-br from-brand-black to-brand-olive-dark text-white p-6 h-full border border-brand-gold/10">
                                ...
                                <ul className="space-y-3">
                                    {[
                                        ...
                                        <motion.li
                                            ...
                                            className="flex items-start gap-2.5 text-sm text-brand-olive-light"
                                        >
