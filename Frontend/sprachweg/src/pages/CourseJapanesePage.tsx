...
import { Check, ChevronRight, Clock, Target, Award, Shield, Languages, Star, AlertCircle, BookOpen } from 'lucide-react';
...
const CourseLevelCard: React.FC<CourseLevelCardProps> = ({ level, onEnroll }) => (
  ...
  >
    ...
    <div className="flex flex-1 flex-col p-8">
      ...
      <button onClick={onEnroll} className="flex w-full items-center justify-center gap-2 rounded-lg bg-brand-red py-4 text-sm font-bold text-white shadow-lg transition-all hover:bg-brand-red-hover hover:shadow-xl">
        Enroll Now
...
const faqs = [
  ...
  { id: '3', question: 'Do you provide study materials?', answer: 'Yes! All levels include comprehensive study materials and mock tests.' },
  { id: '4', question: 'Can I work in Japan after learning Japanese?', answer: 'Yes! Japan has a growing demand for skilled foreign professionals. JLPT N2/N1 certification significantly boosts your employability.' },
];
...
const fallbackJapaneseCourse: SkillCourse = {
  ...
  levels: [
    { name: 'N5', duration: '3 Months', price: '₹17,999', features: ['Hiragana & Katakana mastery', 'Basic Kanji (100 characters)', 'Simple sentence patterns', 'Self-introduction & greetings', 'Numbers & counting systems', 'Basic listening practice'], outcome: 'Read and understand basic Japanese, handle simple everyday conversations.', examPrep: { title: 'JLPT N5 Preparation', details: 'Complete preparation for JLPT N5 certification exam.' } },
    { name: 'N4', duration: '4 Months', price: '₹21,999', features: ['Intermediate Kanji (300 characters)', 'Grammar patterns & conjugations', 'Reading comprehension', 'Conversational Japanese', 'Writing practice', 'Cultural context'], outcome: 'Understand basic Japanese in everyday situations and read simple passages.', examPrep: { title: 'JLPT N4 Preparation', details: 'Targeted preparation for JLPT N4 certification.' } },
    { name: 'N3', duration: '5 Months', price: '₹27,999', features: ['Advanced Kanji (650 characters)', 'Complex grammar patterns', 'Business Japanese basics', 'Newspaper reading', 'Presentation skills', 'Advanced listening'], outcome: 'Understand Japanese used in everyday situations to a certain degree.', examPrep: { title: 'JLPT N3 Preparation', details: 'Comprehensive coaching for JLPT N3 exam success.' } }
  ]
};

const stats = [
  { value: '🇯🇵', label: '3rd Largest Economy' },
  { value: '125M+', label: 'Native Speakers' },
  { value: '2000+', label: 'Kanji Characters' },
  { value: '¥700K+', label: 'Avg. IT Salary (Monthly)' },
];
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        ...
        if (japaneseCourse) { setCourse(japaneseCourse); } else { setCourse(fallbackJapaneseCourse); }
      } catch (error) {
    ...
    <div className="relative min-h-screen bg-brand-white text-brand-black">
        ...
        <div className="mx-auto max-w-7xl grid lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-brand-gold/30 bg-brand-gold/10 px-4 py-1.5">
              ...
              <span className="text-sm font-semibold text-brand-gold">JLPT Certification Path</span>
            </div>
            <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight text-brand-white mb-6 leading-tight">
              Master Japanese,<br className="hidden sm:block"/> Shape <span className="text-brand-red">Your Future</span>
            </h1>
          ...
          >
            ...
            <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 text-brand-white">
              <div className="text-2xl md:text-3xl font-bold">日本 — Japan</div>
              <div className="text-sm md:text-base text-brand-off-white font-medium">Land of the Rising Sun & Innovation</div>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <main id="main-content" className="relative z-10">
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8 bg-brand-off-white">
          <div className="mx-auto max-w-6xl">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="text-center mb-12">
              <motion.h2 variants={fadeInUp} className="text-3xl sm:text-4xl font-bold text-brand-black mb-4">Why Learn Japanese?</motion.h2>
              <div className="w-16 h-1 bg-brand-red mx-auto rounded"></div>
            </motion.div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { icon: <Shield className="h-8 w-8 text-brand-gold" />, title: 'Tech & Innovation', text: 'Japan leads in robotics, AI, and automotive. Japanese opens doors to Sony, Toyota, and Nintendo.' },
                { icon: <Languages className="h-8 w-8 text-brand-gold" />, title: 'Anime & Culture', text: 'Experience manga, anime, and traditional arts in their original language.' },
                { icon: <Star className="h-8 w-8 text-brand-gold" />, title: 'Career Boost', text: 'JLPT certification is recognized worldwide and valued by Japanese multinationals.' },
                { icon: <BookOpen className="h-8 w-8 text-brand-gold" />, title: 'Study in Japan', text: 'Access MEXT scholarships and study at top Japanese universities tuition-free.' },
              ].map((item, idx) => (
        ...
        <section className="px-4 py-24 sm:px-6 lg:px-8 bg-brand-white">
          <div className="mx-auto max-w-7xl">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-16 text-center">
              ...
              <p className="mt-4 text-lg text-brand-olive-dark">Structured learning paths by Skyline from beginner to native fluency.</p>
            </motion.div>
        ...
        <section className="bg-brand-black py-20 px-4 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold text-brand-gold mb-6">Start Your Japanese Journey Today</h2>
          <p className="text-brand-off-white max-w-2xl mx-auto mb-10 text-lg">Connect with culture, business, and innovation through language with Skyline.</p>
          <button
            onClick={() => { setSelectedLevelName('N5'); setIsEnrollModalOpen(true); }}
