...
const findExistingRegistration = async (userId, webinarId) =>
    WebinarRegistration.findOne({ userId, webinarId });

const syncWebinarRegistrationCalendar = async (webinarId) => {
...
const upsertRegistrationFromAttempt = async (attempt) => {
...
const findWebinarAttempt = async (attemptId, transactionId) => {
    const orConditions = [];
    ...
    try {
        ...
        if (existingRegistration && ['submitted', 'accepted'].includes(existingRegistration.status)) {
            return res.status(409).json({
                message: 'You have already registered for this webinar or your registration is pending approval.',
            });
        }
        ...
        if (!Number.isFinite(baseAmount) || baseAmount <= 0) {
            return res.status(400).json({
                message: 'The selected webinar fee is not configured correctly for checkout.',
            });
        }
...
export const getAdminWebinarRegistrations = async (req: Request, res: Response) => {
    try {
        ...
        const registrations = await WebinarRegistration.find(filters)
            .populate(
                'userId',
                'name email phoneNumber germanLevel guardianName guardianPhone qualification dateOfBirth avatar role createdAt isVerified'
            )
...
export const updateWebinarRegistrationStatus = async (req: Request, res: Response) => {
    try {
        ...
        if (previousStatus === 'accepted' || requestedStatus === 'accepted') {
            try {
                await syncWebinarRegistrationCalendar(registration.webinarId?._id || registration.webinarId);
        ...
        return res.status(200).json({
            ...
            registration: refreshedRegistration
                ? withResolvedPaymentFields(refreshedRegistration.toObject())
                : null,
        });
    } catch (error) {
